...
namespace Suckless.Asserts
{
    ...
    {
        public static ref readonly Metadata<TValue> Default<TValue>(in this Metadata<TValue> metadata, string message = null)
        {
            if (!EqualityComparer<TValue>.Default.Equals(metadata.Value, default)) 
            {
                ...
                throw new ArgumentException(message == null 
                    ? $"The {metadata.Name} that contain {strValue} must be default."
                    : message);
            }
